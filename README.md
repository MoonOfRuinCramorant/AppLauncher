...
- 🧲 **边缘隐藏** — 右键悬浮球可暂时隐藏到屏幕右/上边缘，鼠标移近边缘自动呼出，移开自动收回
- 🎨 **自定义图标** — 右键悬浮球可上传本地图片作为球体图标，随时恢复默认
...
- **悬浮球** — 拖拽移动位置（自动记忆）；单击打开快捷面板；右键弹出菜单（显示主窗口 / 隐藏到屏幕右侧或顶部 / 更换或恢复图标 / 关闭悬浮球）；主窗口关闭后悬浮球仍在
- **边缘隐藏** — 右键选择「隐藏到屏幕右侧/顶部」后，悬浮球贴到屏幕边缘只露出一点；鼠标移到该边缘即自动呼出，移开片刻自动收回；右键「取消隐藏」恢复自由位置
- **开机自启** — 设置中开启「开机自动启动」
- **配置文件位置**
